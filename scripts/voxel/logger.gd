...
# General purpose logging system with file output and console filtering
enum LogLevel {
	DEBUG = 0,
	INFO = 1,
	WARNING = 2,
...
static func _init_logging():
	...
	var log_path = logs_dir + "/game_log_%s.log" % timestamp
	log_file = FileAccess.open(log_path, FileAccess.WRITE)
	
	if log_file:
		log_file.store_line("=== Game Log Started: %s ===" % Time.get_datetime_string_from_system())
		log_file.flush()
		print("Logger: Logging to ", log_path)
	else:
		print("Logger: Failed to open log file at ", log_path)
...
static func close_log():
	if log_file:
		log_file.store_line("=== Game Log Ended: %s ===" % Time.get_datetime_string_from_system())
		log_file.close()
		log_file = null
